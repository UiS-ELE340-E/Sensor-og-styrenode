...
// Vent s�  40msek, krav: minst 30msek
//---------------------------------------------
        j = 400;
        while(j-- > 0){
        	vent_100usek();
...
//---------------------------------------------
        ...
        LCD_kommando(0x0E);       // Skjerm og visar p�, blink av
        LCD_kommando(0x01);       // Slett skjerm og k�yr heim visaren

        //  Vent s� 4msek, krav: 1.53msek
//---------------------------------------------
        j = 40;
        while(j-- > 0){
        	vent_100usek();
...
//-----------------------------------------------------------------------
// Ymse sm� metodar for LCD-handtering
//-----------------------------------------------------------------------
...
// Etter � ha sendt data el.kommando til LCD, m� ein venta i > 40usek, b�r i praksis venta 100usek
         vent_100usek();
}
...
void LCD_tal2siff_4( uint16_t tal ) {
        ...
        uint16_t siffer;       // sifra ut p� skjermen
        uint8_t teikn;
...
// Sett n� opp sekvens iflg. tidsdiagram for LCD for skriving av kommando (RS = 0)
...
//
    ...
    GPIOA->ODR = databussverdi;  //Legg ut kommando p� databussen

        vent_400nsek();  //M� vera med
...
void LCD_skrivesyklus_kommando(uint8_t databussverdi) {
    ...
    GPIOA->ODR = databussverdi; //Legg ut ascii-teikn p� databussen
...
//
//// Sett n� opp sekvens iflg. tidsdiagram for LCD for skriving av kommando (RS = 0)
//    GPIOA->ODR &= (0xFF00 | databussverdi);  //Legg ut kommando p� databussen
//
//    GPIOC->ODR &= (0xFFF0 | LCD_Write | LCD_RS_data);
////
//    GPIOC->ODR &= (0xFFF0 | LCD_Write | LCD_RS_data | LCD_Enable);
...
//
//	// Sett n� opp sekvens iflg. tidsdiagram for LCD for skriving av kommando (RS = 0)
//    GPIOA->ODR &= (0xFF00 | databussverdi); //Legg ut ascii-teikn p� databussen
//
//    GPIOC->ODR &= (0xFFF0 | LCD_Write | LCD_RS_kommando);
//	//
//    GPIOC->ODR &= (0xFFF0 | LCD_Write | LCD_RS_kommando | LCD_Enable);
