...
// Fil: SPI1_metodar.c
// m.t.081014
// Metodar for SPI1 og dei modulane som han er knytta til, nemleg
// gyroskopet L3GD20 p� plattformkortet.
//---------------------------------------
...
//S� oppsett av GPIO-pinnane PA5 (SCLK), 6 (MISO) og 7 (MOSI) som blir brukt av SPI1-modulen
//------------------------------------------
//Deklarasjon av initialiseringsstrukturen.
...
//S� oppsett av GPIO-pinnen PE3 (CS) som blir brukt av SPI1-modulen
//------------------------------------------
    //Deklarasjon av initialiseringsstrukturen.
...
//    GPIOE->ODR = GPIOE->ODR & (~(GPIO_Pin_3));
////        SPI2->DR = data;
//	SPI_SendData8(SPI1, 0xA6);//0x8F); //RD, MS, adr_tempreg
//	SPI_SendData8(SPI1, 0xAA); //Vilk�rleg
//	while(SPI1->SR & SPI_SR_FTLVL);
//	while(SPI1->SR & SPI_SR_BSY);
//    GPIOE->ODR = GPIOE->ODR | GPIO_Pin_3;
...
//
//       j = (int8_t)SPI_ReceiveData8(SPI1);
//    }
//    return j; // ((SPI1->SR & SPI_SR_FRLVL)>>9);//(int8_t)SPI_ReceiveData8(SPI1); //j; SPI2->DR;
//}
//void Exp_click_sokkel1_oppstart(void) {
